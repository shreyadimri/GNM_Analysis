...
#check what is excluded i.e. flagged or for SMDH only (as of now 22 rows are excluded)
# excluded_rows<- anti_join(dataset_analysis, dataset_lnRR)
...
#check what is excluded, 7 values that have a negative mean/SD
# excluded_rows <- anti_join(dataset_analysis, dataset_lnRR)
...
# 4 effect size where SMDH needs to be estimated from test statistics and 9 from GNM_355 where repeated estimates over days for haemoglobin are present and I cannot combine them as I did for chick mass over months.. since the sample is the same and these are the repeated measures of the sample sample.
